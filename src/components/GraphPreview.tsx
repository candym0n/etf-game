...
interface MarketPeriodData {
    ...
    equity_etfs: Record<string, number[]>;
    bond_etfs: Record<string, number[]>;
}
...
// Assign unique colors to series, and special style for custom ETF
const applySeriesColors = (series: LineSeries[]): LineSeries[] => {
    let paletteIndex = 0;
...
const GraphPreview: React.FC<GraphPreviewProps> = ({
    ...
    const { dates, stockSeriesFull } = useMemo(() => {
        ...
        const stockData: LineSeries[] = stocks
        ...
        const equityETFData: LineSeries[] = equityETFs
            .filter((ticker) => ticker in periodData.equity_etfs)
            .map((ticker) => ({
                id: ticker,
                label: ticker,
                data: periodData.equity_etfs[ticker].filter(
                    (_, i) => i % DATA_SCALE_DOWN_FACTOR === 0,
                ),
                showMark: false,
            }));

        const bondETFData: LineSeries[] = bondETFs
            .filter((ticker) => ticker in periodData.bond_etfs)
            .map((ticker) => ({
                id: ticker,
                label: ticker,
                data: periodData.bond_etfs[ticker].filter(
                    (_, i) => i % DATA_SCALE_DOWN_FACTOR === 0,
                ),
                showMark: false,
            }));
        ...
        return {
            dates: scaledDates,
            stockSeriesFull: coloredSeries,
        };
    }, [activePeriod, stocks, equityETFs, bondETFs, showETF, etfStocks]);
